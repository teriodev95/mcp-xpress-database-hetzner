...
-- Descripción: Inserta snapshot de cobranza por agencia (VERSIÓN FINAL)
--              Captura TODAS las agencias (352) incluso sin pagos
--              Usa vw_datos_cobranza para métricas (SIN duplicados de multas)
--              Usa tabla multas separada para obtener multas
...
BEGIN
    ...
    -- IMPORTANTE: vw_datos_cobranza ahora SIN duplicados porque las multas
    --             ya fueron eliminadas de pagos_v3
    ...
    SELECT
        ...
        -- Cobranza desde vw_datos_cobranza (pre-agregada, SIN duplicados de multas)
    ...
    -- NOTA: Ya no tiene duplicados porque eliminamos multas de pagos_v3
    LEFT JOIN (
        SELECT
            agencia,
...
-- 1. ✅ Tabla multas creada y separada
-- 2. ✅ Multas eliminadas de pagos_v3 (ya no causan duplicados en vw_datos_cobranza)
-- 3. ✅ Multas NO se insertan en pagos_dynamic (trigger modificado)
-- 4. ✅ SP usa tabla multas para obtener total de multas
-- 5. ✅ vw_datos_cobranza ahora SIN duplicados (porque no hay multas en pagos_v3)
-- 6. ✅ Mantiene todas las 352 agencias
-- 7. ✅ Velocidad: 1-3 segundos
...
-- MULTAS:
-- 1. Se insertan en tabla multas directamente (sp_insertar_multa) o via API
-- 2. NO van a pagos_v3 (para evitar duplicados en vw_datos_cobranza)
-- 3. NO van a pagos_dynamic (trigger lo previene)
-- 4. Este SP las obtiene desde tabla multas

-- PAGOS NORMALES:
-- 1. Se insertan en pagos_v3
-- 2. Trigger copia/actualiza en pagos_dynamic
-- 3. vw_datos_cobranza calcula métricas desde pagos_dynamic
-- 4. Este SP usa vw_datos_cobranza para cobranza_pura y excedente

-- =====================================================
-- IMPORTANTE: PREREQUISITOS
-- =====================================================

-- Antes de usar este SP, asegúrate de haber ejecutado:
-- 1. crear_tabla_multas.sql
-- 2. triggers_separar_multas_CORREGIDO.sql
-- 3. MODIFICAR_trg_pagos_v3_before_insert.sql
-- 4. Eliminar multas existentes de pagos_v3:
--    DELETE FROM pagos_v3 WHERE Tipo = 'Multa';
...
-- WHERE semana = 3 AND anio = 2026 AND agencia = 'AGD051';
--
-- Debe mostrar:
-- - total_cobranza_pura: correcto (sin duplicados)
-- - monto_excedente: correcto (sin duplicados)
-- - multas: 650.00 (desde tabla multas)
